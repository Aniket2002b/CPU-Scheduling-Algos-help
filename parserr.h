...
void parse_processes(){
    ...
    int process_arival_time, process_service_time;
    ...
    for(int i = 0; i < process_count; i++){
        ...
        getline(stream, temp_str,',');
        process_arival_time = stoi(temp_str);
        getline(stream,temp_str,',');
        process_service_time = stoi(temp_str);


        processes.push_back(make_tuple(process_name,process_arival_time, process_service_time));
